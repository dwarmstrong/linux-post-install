...
NAME="Numero Uno Ubuntu Boot"
SHORT_NAME="NUUB"
BLURB="the next step after first boot"
...
clear
echoCyan "$( penguinista ) .: $NAME ($SHORT_NAME) -- $BLURB :."
cat << _EOF_
...
testRoot() {
local message="$SHORT_NAME requires ROOT privileges to do its job."
if [[ $UID -ne 0 ]]; then
...
testConnect() {
local message="$SHORT_NAME requires an active network interface."
...
clear; header; sleep 4
apt -y install $sound $net $av $image $doc $devel && footer
}
